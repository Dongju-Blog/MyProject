...
function useAuthorityInit() {
  ...
  const _TARGET_URL: { [prop: string]: { [prop: string]: { url: string; message: string } } } = {
		ADMIN: {
    ...
    if (currentPage) {
      if (storeUserAtom.role && storeUserAtom.status) {
        if ( currentPage.role.includes(storeUserAtom.role) && currentPage.status.includes(storeUserAtom.status)){
          setIsValidPage(() => true)
        } else {
