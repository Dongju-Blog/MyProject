...
function Pagination({currentPage, maxPage, baseUrl, queryString}: PaginationPropsType) {
  ...
  const nextOnClickHandler = () => {
    const targetPage = pages[pages.length - 1] + 2 < maxPage ? pages[pages.length - 1] + 2 : maxPage
    if (baseUrl.includes("?")) {
      router.push(`${baseUrl}&${queryString}=${targetPage}`)
    } else {
      router.push(`${baseUrl}?${queryString}=${targetPage}`)
...
const buttonWrapperCSS = ({target, current}: {target: number; current: number}) => {
  return css`
    width: 24px;
    height: 24px;
    display: flex;
