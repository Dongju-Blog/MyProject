...
const NotificationOverlay = (props: any) => {
  const [stackNotificationAtom, setStackNotificationAtom] =
    useAtom(stackNotification);
  ...
  const notiProcessor = Object.keys(stackNotificationAtom).map((key) => {
    ...
    return (
      <Notification
        ...
        id={key}
        width={
          stackNotificationAtom[numKey].width
            ? stackNotificationAtom[numKey].width
            : "320px"
        }
        height={
          stackNotificationAtom[numKey].height
            ? stackNotificationAtom[numKey].height
            : "320px"
        }
        content={stackNotificationAtom[numKey].content}
        duration={
          stackNotificationAtom[numKey].duration
            ? stackNotificationAtom[numKey].duration
            : 3000
        }
  ...
  return (
    <div css={backdropCSS}>
      <div css={notificationPlace}>{notiProcessor}</div>
    </div>
  );
};
