...
function SourceCodeCreate({updateData}: SourceCodeCreatePropsType) {
  const [title, setTitle] = useState<string>(updateData ? updateData.title : '')
  const [description, setDescription] = useState<string>(updateData ? updateData.description : '')
  const [image, setImage] = useState<Blob>()
  const [file, setFile] = useState<Blob>()
  const [rootName, setRootName] = useState<string>(updateData ? updateData.rootName : '')
  const [imageUrl, setImageUrl] = useState<string>(updateData ? updateData.imageUrl : '')
  ...
  const inputImageOnChangeHandler = async (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const inputFileOnChangeHandler = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      const file = e.target.files[0]
      setFile(() => file)
  ...
  const postSourceCodeMutation = useMutation(
    ({ body }: { body: postSourceCodeBodyType }) =>
    postSourceCodeAPI({ body })
  );
  ...
  const submitHandler = async () => {
    ...
    if (updateData && updateData.id) { 
      putMutateHandler({formData})
    } else {
      postMutateHandler({formData})
    }
  ...
  return (
    <div css={wrapperCSS}>
      ...
      <div css={imageWrapperCSS} onClick={inputImageOnClickHandler}>
          ...
          {imageUrl ? <img src={imageUrl} css={css`height: 100%; width: 100%; object-fit: cover;`}/> : <img src={"/assets/Thumbnail.png"} css={css`filter: invert(80%); width: 70%; height: auto;`}/>}
          <input ref={inputImageRef} type="file" accept='image/*' onChange={inputImageOnChangeHandler} css={css`display: none;`}/>
        
        
      </div>

      <div css={textWrapperCSS}>
          <Input value={title} onChange={(e) => {setTitle(() => e.target.value)}} placeholder={"Input the title, please."} theme={'default'} css={css`height: 38px;`}></Input>
          <Input placeholder={"Upload zip file only."} value={rootName} theme={'default'} css={css`height: 38px;`} disabled={true}>
            <Input.Right>
              <input ref={inputFileRef} formEncType="multipart/form-data" onChange={inputFileOnChangeHandler} type="file" accept='.zip' css={css`display: none;`}/>
              <Button onClick={inputFileOnClickHandler} theme={"default"}>Explore</Button>
            </Input.Right>
          </Input>
          
          
          <Textarea value={description} onChange={(e) => {setDescription(() => e.target.value)}} placeholder={"Input the description, please."} theme={'default'} customCss={css`flex: 1;`} />
        <div css={buttonWrapperCSS}>
        <Button onClick={() => {router.push('/playground')}} theme={"grey"} css={css`width: 96px;`}>Cancel</Button>
        <Button onClick={submitHandler} theme={"default"} css={css`width: 96px;`}>Submit</Button>
        </div>
